    ...
    public function save()
    {
        ...
        if ($request_server_to_cache) {
            ...
            if (file_put_contents($file_name, $request_server_to_cache['content'])) {
				...
				if($request_server_to_cache['headers']['Via'])
				{
    ...
    public function validate()
    {
		...
		**/
        if (isset($this->request_client_to_cache->headers['last-modified'])) {
            if (!isset($assets_header) OR strtotime($assets_header['last-modified']) != strtotime($this->request_client_to_cache->headers['last-modified'])) {
                return false;
            } else {
                return true;
            }
		...
		**/
        if (isset($this->request_client_to_cache->headers['etag'])) {
            if (!isset($assets_header) OR strtotime($assets_header['etag']) != strtotime($this->request_client_to_cache->headers['etag'])) {
                return false;
            } else {
                return true;
            }
		...
		**/
		if(isset($this->request_client_to_cache->headers['cache-control']))
		{
			...
			$search = array( 'no-cache', 'no-store', 'max-age');
		...
		# need to reuse above code for pragma cache control (by moving into separate method) but let is be for now
